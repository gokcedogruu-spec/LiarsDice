...
function startNewRound(room, isFirst = false, startIdx = null) {
    ...
    // 1. Сначала отправляем кубики и состояние стола (чтобы отрисовался интерфейс игры)
    let safety = 0;
    ...
    broadcastGameState(room); // Отрисовываем стол для всех
    
    // 2. ПРОВЕРКА ПАССИВОК И ИНТРО
    let introDelay = 0;
    if (room.config && room.config.crazy && isFirst) {
        ...
        if (passives.length > 0) {
            ...
            introDelay = passives.length * 3500;
        }
    }
    
    // 3. Откладываем старт таймера и начало хода, пока идет интро
    setTimeout(() => {
        io.to(room.id).emit('gameEvent', { text: `🎲 РАУНД!`, type: 'info' });
        resetTurnTimer(room); // Запускаем таймер и ботов
    }, introDelay);
}
